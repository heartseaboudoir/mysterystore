...
$shequ_ids = "6";//要结款区域门店
$max_initial_storage_id = 18326;//期初入库子表最大ID【搜hii_store_in_stock_detail】

######################################################################
#区域名           #区域ID            #max_initial_storeage_id        #
######################################################################
#福州             #7                 #3627                           #
#深圳             #11                #6269                           #
#厦门             #15                #3812                           #
...
// 获取最后一个操作的门店
$sql = "
select GSNSI.* from " . DB_PRE . "goods_store_new_swift_index GSNSI
left join hii_store S on S.id=GSNSI.store_id
where GSNSI.year = " . $prev_year . " and GSNSI.month = " . $prev_month . " and S.shequ_id in (".$shequ_ids.")
order by GSNSI.store_id desc limit 1
";
$data = $db->query($sql);
$prev_store_id = 0;
...
// 获取将要操作的门店
$sql ="select * from " . DB_PRE . "store where `status` = 1 and shequ_id in ({$shequ_ids}) and id > " . $prev_store_id . " order by id asc limit 1";
$data = $db->query($sql);
if (!$data) {
...
if ($this_month_goods_store_snapshot) {
    ...
    $sql .= "where SIS.store_id2={$store_id} and SIS.s_in_s_type<>2 and SISD.s_in_s_d_id>{$max_initial_storage_id} and SIS.ptime between {$start_statistics_time} and {$end_statistics_time} and SISD.goods_id in ({$goods_in_sql}) ";
    ...
    foreach ($this_month_goods_store_snapshot as $key => $val) {
        ...
        $in_num = 0;//入库数量【不计算盘盈数量,不计期初入库数量】
