...
/***/ function(module, exports, __webpack_require__) {
	...
	var _Confirm = __webpack_require__(2);
	...
	var _ScrollLoad = __webpack_require__(6);
	 ...
	 *   msg: 'heheh',
	 *   close: function () {
	 *     console.log('is my close method.');
	 *   },
	 *   confirm: function () {
	 *     console.log('is my confirm method.');
	 *   }
	 * }, 'body');
	 */
	window.ConfirmFactory = (0, _Utils.ComponentFactory)(_Confirm2.default);
	
	/**
	 *  ScrollLoadRender({
	 *    loadData: loadDataFactory('vendor/data.html', $(this).data('type'))
	 *  }, this)
	 */
	window.ScrollLoadRender = (0, _Utils.ComponentRender)(_ScrollLoad2.default);
	 ...
	 *      }, '#tabs');
	 */
	window.TabsRender = (0, _Utils.ComponentRender)(_Tabs2.default);
	
	window.Timer = _Timer2.default;
	
	// global
	window.Loading = new _Loading2.default();
	
	$(function () {
	  window.Loading.$mount().$appendTo('body');
	});
	
	// export const ConfirmFactory = ComponentFactory(Confirm)
	// export const TabsFactory = ComponentFactory(Tabs)
...
/***/ function(module, exports) {
	...
	var NEW = exports.NEW = function NEW(f) {
	  var obj = void 0,
	      ret = void 0,
	      proto = void 0;
	...
	/**
	 * 分离 data 和 methods 参数
	 *
	 * @param options
	 * @returns {{data: {}, methods: {}}}
	 */
	function extractParam(options) {
	  var methods = {};
	  var data = {};
	...
	// template 是自己的
	var ComponentFactory = exports.ComponentFactory = function ComponentFactory(fn) {
	  return function (options, selector) {
	    var vueOption = Object.assign({}, extractParam(options));
	
	    return NEW(fn, vueOption).$mount().$appendTo(selector);
	  };
	};
	
	// template 在页面上了
	var ComponentRender = exports.ComponentRender = function ComponentRender(fn) {
	  return function (options, el) {
	    var vueOption = Object.assign({}, extractParam(options));
	
	    vueOption.el = el;
	    return NEW(fn, vueOption);
	  };
	};
	
	// 单例
	var ComponentSingle = exports.ComponentSingle = function ComponentSingle(fn, options, selector) {
	  var vueOption = Object.assign({}, extractParam(options));
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Confirm = _vue2.default.extend({
	  data: function data() {
	    return {
	      msg: '',
	      display: true
	  ...
	  methods: {
	    _handleClose: function _handleClose(evt) {
	      this.display = this.close && !!this.close() || false;
	    },
	    _handleConfirm: function _handleConfirm(evt) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Tabs = _vue2.default.extend({
	  ...
	  ready: function ready() {
	    ...
	    var $root = $(this.$el);
	    this.$tabTitles = $root.find('.weui_navbar_item');
	    this.$tabs = $root.find('.weui_tab_bd');
	    this.tabTitleClass = 'weui_bar_item_on';
	    this.tabsClass = 'weui_tab_bd_on';
	
	    this.$tabTitles.each(function (index, el) {
	      $(el).click(function () {
	  ...
	  methods: {
	    itemClick: function itemClick(index) {
	      this.$tabs.removeClass(this.tabsClass).eq(index).addClass(this.tabsClass);
	      this.$tabTitles.removeClass(this.tabTitleClass).eq(index).addClass(this.tabTitleClass);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ScrollLoad = _vue2.default.extend({
	  data: function data() {
	    return {
	      distance: 100,
	      containerHeight: 0,
	      ...
	      isLoading: false,
	      isEnd: false,
	      loadingTemplate: ' <div class="weui_cells tabs-loading">加载中...</div>',
	      emptyTemplate: ' <div class="weui_cells tabs-empty"></div>',
	      endTemplate: ' <div class="weui_cells tabs-end">没有更多了</div>'
	    };
	  },
	  ready: function ready() {
	    $('html').addClass('fullscreen');
	    this.$root = $(this.$el);
	    this.loadingTemplate = $(this.loadingTemplate);
	    this.containerHeight = this.$root.height();
	    this._calceTotalHeight();
	    this.$root.on('scroll', this._handlerScroll);
	  },
	
	  methods: {
	    _handlerScroll: function _handlerScroll() {
	      var _this = this;
	      ...
	      if (this._canLoad()) {
	        console.log('fwfewfw');
	        this._loading();
	        this.loadData && this.loadData(function (data) {
	          _this._unloading();
	
	          if ((typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object') {
	            _this.isEnd = data.isEnd;
	            _this.$root.off('scroll', _this._handlerScroll);
	            $(_this.endTemplate).appendTo(_this.$root);
	          } else if (data === '') {
	            $(emptyTemplate).appendTo(_this.$root);
	            _this._calceTotalHeight();
	          } else {
	            $(data).appendTo(_this.$root);
	            _this._calceTotalHeight();
	          }
	        });
	      }
	    },
	    _canLoad: function _canLoad() {
	      return !this.isEnd && !this.isLoading && this.totalHeight - this.scrollTop - this.containerHeight < this.distance;
	    },
	    _calceTotalHeight: function _calceTotalHeight() {
	      var margin = 0;
	      this.totalHeight = [].reduce.call(this.$root.children(), function (previousValue, item) {
	        margin = parseInt($(item).css('margin-top'));
	        // Note: script tag height !== 0
	        return previousValue + (item.tagName.toLowerCase() === 'script' ? 0 : $(item).outerHeight());
	      }, 0);
	      this.totalHeight += (this.$root.children().length + 1) * margin;
	    },
	    _loading: function _loading() {
	      this.isLoading = true;
	      this.loadingTemplate.appendTo(this.$root);
	    },
	    _unloading: function _unloading() {
	      this.isLoading = false;
	      this.loadingTemplate.remove();
	    },
	    load: function load() {
	      var _this2 = this;
	      ...
	      if (!this.isEnd && !this.isLoading) {
	        ...
	        this.loadData && this.loadData(function (data) {
	          ...
	          if ((typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object') {
	            _this2.isEnd = data.isEnd;
	            _this2.$root.off('scroll', _this2._handlerScroll);
	            $(_this2.endTemplate).appendTo(_this2.$root);
	          } else {
	            $(data).appendTo(_this2.$root);
	            _this2._calceTotalHeight();
	          }
	        });
	      }
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Toast = _vue2.default.extend({
	  data: function data() {
	    return {
	      msg: '',
	      display: false,
	  ...
	  computed: {
	    classObject: function classObject() {
	      return {
	        'weui_toast': true,
	        'weui_toast_plain': !this.isLoading
	      };
	    },
	  ...
	  template: '<div v-if="display" class="weui_loading_toast">\
	              <div class="weui_mask_transparent" v-if="isModal"></div>\
	              <div v-bind:class="classObject">\
	                <div class="weui_loading" v-if="isLoading">\
	  ...
	  methods: {
	     ...
	     * {
	     *   msg: '数据加载中',
	     *   autoClose: 2000, 两秒后关闭
	     *   isLoading: [ true | false ], 是否有加载图标
	     *   onClose: function, fire while close
	     * }
	     * @param option
	     */
