...
class PublicController extends Controller {
    ...
    public function wx_pay_notify(){
        ...
        unset($new_data['id']);
        if(M('WechatPayRecord')->add($info)){
            $order = D('Addons://Order/Order')->set_pay($order_sn, 1, $data['out_trade_no'], $data['attach']);
    ...
    public function ali_pay_notify(){
        ...
        if($data['trade_status'] == 'TRADE_SUCCESS'){
            ...
            $price = empty($data['total_amount']) ? 0 : $data['total_amount'];
            ...
            $new_data = array_merge($info, $idata);
            unset($new_data['id']);
            if(M('AlipayData')->add($info)){
                D('Addons://Order/Order')->set_pay($order_sn, 2, $data['out_trade_no'], $data['body']);
            }
        }
        echo 'success';
        exit;
    ...
    public function kd_notify(){
        if(IS_POST){
            $req_data = $_POST;
            M('Test')->add(array('val' => json_encode($req_data), 'type' => 'kd_notify', 'create_time' => date('Y-m-d H:i:s', NOW_TIME)));
            ...
            if(!$Api->check_crypt($req_data['DataSign'], $req_data['RequestData'])){
                M('Test')->add(array('val' => 'check_crypt', 'type' => 'kd_notify', 'create_time' => date('Y-m-d H:i:s', NOW_TIME)));
                exit;
            }
            if($req_data['RequestType'] == '101'){
                foreach($request_data['Data'] as $v){
                    ...
                    M('OrderExpressLog')->where(array('no' => $v['LogisticCode']))->save(array('data' => json_encode($item)));
                    M('Test')->add(array('val' => 'Order_save', 'type' => 'kd_notify', 'create_time' => date('Y-m-d H:i:s', NOW_TIME)));
                }
                $datas = array(
