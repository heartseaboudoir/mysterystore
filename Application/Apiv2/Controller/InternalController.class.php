...
class InternalController extends ApiController {
     ...
     * @remark 接口附加返回参数<br><br>
                ...
                [total] => 总的数据条数<br><br>
     */
    public function get_store_lists2(){
        $this->check_token();
    ...
    public function get_store_lists(){
        ...
        if ($this->group_id != 1) {
            ...
            $_my_store = M('MemberStore')->where(array('uid' => $this->uid, 'type' => 1, 'group_id' => 2))->select();



            if ($_my_store) {
                foreach ($_my_store as $key0 => $val0) {
                    $my_store[] = $val0['store_id'];
                }
            }

            $my_shequ = M('MemberStore')->where(array('uid' => $this->uid, 'type' => 2, 'group_id' => 2))->select();
            ...
            $where['id'] = array('in', $my_store);
        }
        
        $where['status'] = array('in', array(1, 2));

        //$this->return_data(1, $where);
    ...
    public function get_store_lists_test(){
        $this->check_token();
        $page = I('page', 1, 'intval');
        $page < 1 && $page = 1;
        $row = 20;
