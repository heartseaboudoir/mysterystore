...
class AddonsModel extends Model {
    ...
    public function getAdminList(){
        $admin = array();
        $db_addons = $this->where("status=1 AND has_adminlist=1")->field('title,name,adminlist_url')->select();
        if($db_addons){
            foreach ($db_addons as $value) {
                $admin[] = array('title'=>$value['title'],'url'=> $value['adminlist_url'] ? $value['adminlist_url'] :"Addons/adminList?name={$value['name']}");
            }
        }
        return $admin;
    }
