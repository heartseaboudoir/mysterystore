...
class DocumentModel extends Model{
    ...
    public function update($data = null){
        ...
        $access_model = M('DocumentGoodsAccess');
        if(!empty($_POST['bind_goods']) && is_numeric($_POST['bind_goods'])){
            if($id){
                $access_data = array(
                    'did' => $id,
                    'goods_id' => $_POST['bind_goods'],
                    'create_time' => NOW_TIME,
                    'update_time' => NOW_TIME
                );
                $access_model->add($access_data);
            }else{
                $access_data = $access_model->where(array('did' => $_id))->find();
                if(!$access_data){
                    $access_data = array(
                        'did' => $_id,
                        'goods_id' => $_POST['bind_goods'],
                    ...
                    $access_model->add($access_data);
                }elseif($access_data['goods_id'] != $_POST['bind_goods']){
                    $access_model->where(array('did' => $_id))->save(array('goods_id' => $_POST['bind_goods'], 'update_time' => NOW_TIME));
                }
            }
        }elseif(empty($_POST['bind_goods']) && !empty($_id)){
            $access_model->where(array('did' => $_id))->delete();
        }
        $TagModel = M('DocumentTagAccess');
        $tag = array();
        if(!empty($_POST['tag'])){
            $_tag = explode(',', $_POST['tag']);
            foreach($_tag as $v){
                $v = trim($v);
                if(!$TagModel->where(array('did' => $_id, 'tag' => $v))->find()){
                    $TagModel->add(array('did' => $_id, 'tag' => $v));
                }
                !in_array($v, $tag) && $tag[] = $v;
            }
        }
        $del_where = array('did' => $_id);
        $tag && $del_where['tag'] = array('not in', $tag);
        $TagModel->where($del_where)->delete();
        //内容添加或更新完成
        return $data;
    }
    ...
    public function checkDocumentType($type = null, $pid = null){
    	$res = array('status'=>1, 'info'=>'');return $res;
		if(empty($type)){
