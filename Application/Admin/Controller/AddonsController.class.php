...
str;
    ...
    public function __call($method, $args) {
        if(substr($method, 0, 3) == 'ex_'){
            $this->assign('_extra_menu', array());
            $_addons = strtolower(I('get._addons', ''));
            if($_addons == 'statistic'){
                $_menus = D('Addons://Statistic/StatisticType')->order('listorder asc')->field('id,title')->select();
                foreach($_menus as &$v){
                    $v['url'] = 'Addons/ex_statistic?_addons=Statistic&_controller=Statistic&_action=index&count_type='.$v['id'];
                }
                $this->assign('_extra_menu',array(
                    '统计管理'=> $_menus,
                ));
            }
            $this->execute(I('get._addons'), I('get._controller'), I('get._action'));
        }else{
