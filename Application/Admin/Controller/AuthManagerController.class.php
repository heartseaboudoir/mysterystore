...
class AuthManagerController extends AdminController{
    ...
    public function func(){
        ...
        */
        ...
        $auth_group = M('AuthGroup')->where( array('status'=>array('egt','0'),'module'=>'admin','type'=>AuthGroupModel::TYPE_ADMIN) )
                                    ->getfield('id,id,title,rules,funcs');

        //$my_funcs = $auth_group[(int)$_GET['group_id']]['funcs'];                            
        ...
        $funcs = M('funcs')->where(array('status' => 1))->order('sort desc')->select();
        ...
        foreach ($funcs as $key1 => $val1) {
            if ($val1['pid'] != 0 && !empty($func0[$val1['pid']])) {
                if (!isset($func0[$val1['pid']]['child'])) {
                    $func0[$val1['pid']]['child'] = array();
                }
                $func0[$val1['pid']]['child'][] = $val1;
            }
        }
        
        $func1 = array_values($func0);
        ...
        */                          
        ...
        $this->assign('funcs', $func1);
        $this->assign('auth_group', $auth_group);
        $this->assign('this_group', $auth_group[(int)$_GET['group_id']]);
        $this->meta_title = '功能授权';
        $this->display('func');
    }   
    ...
    public function writeFuncs(){
        if(isset($_POST['funcs'])){
            sort($_POST['funcs']);
            $_POST['funcs']  = implode( ',' , array_unique($_POST['funcs']));
        }
        $_POST['module'] =  'admin';
