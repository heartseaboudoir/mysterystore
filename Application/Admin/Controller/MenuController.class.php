            ...
            if($data){
                ...
                if($id){
                    ...
                    if(I('_add_base_c') == 1){
                        ...
                        $data['pid'] = $id;
                       if(strpos($data['url'], '_action=index') !== false){ 
                            $url = preg_replace('/([\&\?]_action=)index([\&\?]{0,1})/', '$1[action]$2', $data['url']);
                        }elseif(strpos(trim($data['url'], '/'), '/') !== false){
                            $url = str_replace('/index', '/[action]', $data['url']);
                        }else{
                            $arr = array();
                        }
                        foreach($arr as $v){
                            $v['url'] = str_replace('[action]', $v['url'], $url);
