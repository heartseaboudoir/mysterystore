...
class ArticleController extends AdminController {
    ...
    public function __construct() {
        parent::__construct();
        $_extra_select = M('AuthGroupAccess')->where(array('group_id' => 2,'uid' => UID))->find();
        $this->assign('_extra_select', !$_extra_select);
        $user_wechat = session('user_wechat');
        $this->ukey = $user_wechat['ukey'];
    }
    /**
     * 检测需要动态判断的文档类目有关的权限
     *
     ...
     * @author huajie <banhuajie@163.com>
     */
    protected function getMenu($cate_id = 0){
        //获取动态分类
        ...
        //获取分类id
        !$cate_id && $cate_id        =   I('param.cate_id');
    ...
    public function edit(){
        ...
        /*绑定商品*/
        $bind_access = M('DocumentGoodsAccess')->where(array('did' => $id))->find();
        if($bind_access){
            $bind_goods = M('Goods')->where(array('id' => $bind_access['goods_id']))->field('id,title,cover_id')->find();
            $this->assign('bind_goods', $bind_goods);
        }
    ...
    public function update(){
        !$this->is_admin && $_POST['ukey'] = $this->ukey;
        $res = D('Document')->update();
        if(!$res){
            $this->error(D('Document')->getError());
        }else{
            if(empty($res['id'])){
                if($res['category_id'] == 108){
                    $api = new \User\Client\Api();
                    // 店铺用户的商品发通知
                    $content = I('content', '', 'strip_tags');
                    if($res['category_id'] == 108){
                        $title = '系统通知';//$res['tag'];
                        $notice_c = $res['description'];
                    }else{
                        $title = $res['title'];
                        $notice_c = mb_strlen($content, 'utf-8') > 100 ? msubstr($content, 0, 100, 'utf-8') : $content;
                    }
                    $api->execute('Message', 'add_notice', array('act_uid' => 0, 'act_id' => $res['_id'], 'type' => 'notice','title' => $title, 'content' => $notice_c, 'uid' => 0, 'param' => array(), 'hid' => ''));
                }
            }
