...
class CategoryModel extends Model{
	...
	public function getTree($id = 0, $field = true){
		...
		if($id){
			...
			$id   = $info['id'];
                        if(isset($info['icon'])){
                            $pic_path = get_cover($info['icon'], 'path');
                            $info['pic_url'] = $pic_path ? (get_domain().$pic_path) : '';
                        }
		}
		...
		$list = $this->field($field)->where($map)->order('sort')->select();
                foreach($list as &$v){
                    if(isset($v['icon'])){
                        $pic_path = get_cover($v['icon'], 'path');
                        $v['pic_url'] = $pic_path ? (get_domain().$pic_path) : '';
                    }
                }
	...
	public function getAllChildrenId($cate){
		$field = 'id,name,pid,title,link_id';
		$category = D('Category')->getTree($cate, $field);
		$ids = $this->_get_child($category, true);
		return implode(',', $ids);
	}
        /**
         * 循环获取所有子
         ...
         * @return type
         */
        private function _get_child($category = array(), $all = false){
            $ids = array();
            foreach ($category['_'] as $value) {
                    $ids[] = $value['id'];
                    if($all){
                        $item = $this->_get_child($value, $all);
                        $item && $ids = array_merge($ids, $item);
                    }
            }
            return $ids;
        }
