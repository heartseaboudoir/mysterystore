	...
	if(!empty($grid['href'])){
        ...
        foreach($links as $link){
            ...
            if(preg_match('/^\[([a-z_]+)\]$/',$href,$matches)){
                ...
                $show   =   !empty($array[1])?$array[1]:$value;
                $target   =   isset($array[2])?$array[2]:'';
                // 替换系统特殊字符串
                $href	=	str_replace(
                ...
                $val[]	=	'<a href="'.U($href).'" '.($target ? ('target="'.$target.'"') : '').'>'.$show.'</a>';
            }
        }
        $value  =   implode(' ',$val);
	}
 ...
 * @author 朱亚杰 <zhuyajie@topthink.net>
 */
function extra_menu($extra_menu,&$base_menu, $no_key = array()){
    $add = 0;
    foreach ($extra_menu as $key=>$group){
        // 判断：不添加进入菜单，若判断的子菜单为空的话则不进行排除
        if(!in_array($key, $no_key) || (in_array($key, $no_key) && empty($base_menu['child'][$key]))){
            $add = 1;
        }
        if($add){
...
function get_parent_category($cid){
    ...
    $cates  =   M('Category')->where(array('status'=>1))->field('id,title,pid,allow_publish')->order('sort')->select();
 ...
 * @author 文定明
 */
function get_new_order_no($no_str, $table, $field_str){
    $pre    = $no_str .date("Ymd");
    $sql = "select max($field_str) as f_str from $table where $field_str like '" . $pre . "%'";
    $data = M()->query($sql);
    if($data[0]['f_str'] == '') {
        $out_no = $pre .'00001';
    }else{
