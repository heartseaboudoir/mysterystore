...
function U($url='',$vars='',$suffix=true,$domain=true) {
    ...
    if(C('URL_ROUTER_ON') == true){
        $_url = strtolower($module.'/'.$var['c'].'/'.$var['a']);
        $_reules = C('URL_ROUTE_RULES');
        foreach($_reules as $k => $v){
            $v = strtolower(trim($v, '/'));
            if(substr($v, 0, strlen($module)) != strtolower($module)){
                $v = strtolower($module).'/'.$v;
            }
            if($v == $_url){
                preg_match_all('/:([^\/]+)/is', $k, $match);
                $_k = $_v = array();
                ...
                $url = str_replace($_k, $_v, $k);
                $url = '/'.$url.($vars ?('?'.http_build_query(array_reverse($vars))) : '');
                $is_r = true;
            }
        }
        // End
