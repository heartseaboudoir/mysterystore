<?php
namespace Addons\Push\Lib\Getui;
...
class Api{
	...
	function __construct() {
		...
		define('APPKEY','9NAxQHr2pY5YwuqafgXJm');
		define('APPID','QO7YyM7JmN6RU5dnZ1bnW1');
		define('MASTERSECRET','3nrxnaWZop9l1ydjg22J42');
		define('CID','');
		define('DEVICETOKEN','');
		define('Alias','请输入别名');
	}
	...
	//多推接口案例
	function pushMessageToList($title, $content, $tags)
	{
	...
	//    $contentId = $igt->getContentId($message);
		$contentId = $igt->getContentId($message, $title);	//根据TaskId设置组名，支持下划线，中文，英文，数字
	...
	//群推接口案例
	function pushMessageToApp($title, $content, $tags = array()){
		$igt = new \IGeTui(HOST,APPKEY,MASTERSECRET);
		$template = $this->IGtTransmissionTemplateDemo($content);
		//$template = IGtLinkTemplateDemo();
		//个推信息体
...
//                //$cdt->addCondition(AppConditions::REGION, $provinceList);
		...
		$rep = $igt->pushMessageToApp($message, $title);
                return $rep;
	}
	...
	function IGtLinkTemplateDemo($title, $text, $logo = '', $url = ''){
	...
	function IGtNotificationTemplateDemo($title, $text, $content, $logo = ''){
