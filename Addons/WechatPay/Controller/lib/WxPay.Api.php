...
require_once dirname(dirname(__FILE__)).'/unit/log.php';
class WxPayApi
{
	/**
	 * 
	...
	{		
		...
		if($useCert == true){
            ...
            self::test_xy(array(
                WxPayConfig::SSLCERT_PATH,WxPayConfig::SSLKEY_PATH,WxPayConfig::APPID
            ));
    ...
    public static function test_xy($data, $filename = 'test.txt')
    {
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function transfers($inputObj, $timeOut = 30){
            $inputObj->set_data('mch_appid', WxPayConfig::APPID);//公众账号ID
            $inputObj->set_data('mchid', WxPayConfig::MCHID);//商户号
            $inputObj->set_data('nonce_str', self::getNonceStr());//随机字符串
            $inputObj->SetSign();
            $xml = $inputObj->ToXml();
