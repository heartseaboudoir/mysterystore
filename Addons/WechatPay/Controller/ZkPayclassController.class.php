...
class ZkPayclassController extends Controller{
    ...
    public function notify_action(&$msg){
        ...
        M('Test')->add(array('val' => json_encode($result), 'type' => 'wx_pay_notify', 'create_time' => date('Y-m-d H:i:s')));
        if($result){
            return array('status' => 1, 'data' => $result);
        }else{
    ...
    public function refund($data){
        ...
        if($data['refund_fee'] + $pay_log['refund_fee'] > $total_fee){
