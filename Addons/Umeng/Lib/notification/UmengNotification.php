...
abstract class UmengNotification {
	...
	function send() {
        ...
        //print($result . "\r\n");
        if ($httpCode == "0") {
            return array('status' => 0, 'code' => $httpCode, 'msg' => "Curl error number:" . $curlErrNo . " , Curl error details:" . $curlErr);
          	 // Time out
           	throw new Exception("Curl error number:" . $curlErrNo . " , Curl error details:" . $curlErr . "\r\n");
        } else if ($httpCode != "200") {
           	// We did send the notifition out and got a non-200 response
            return array('status' => 0, 'code' => $httpCode, 'msg' => "Http code:" . $httpCode .  " details:" . $result);
           	throw new Exception("Http code:" . $httpCode .  " details:" . $result . "\r\n");
        } else {
           	return array('status' => 1, 'code' => $httpCode, 'msg' => $result);
        }
    }
