...
class StoreAssignmentApplicationModel extends Model
{
     ...
     * @param $fname 文件名
     */
    public function createStoreAssignmentApplicationListExcel($data, $title, $fname)
    {
        if (!$fname) {
            return false;
        }
        ...
        $objActSheet->setCellValue('B' . $rowNum, '调拨单号');
        ...
        $objActSheet->setCellValue('G' . $rowNum, '调拨门店');
        $objActSheet->setCellValue("H" . $rowNum, '发货门店');
        $objActSheet->setCellValue("I" . $rowNum, '售价金额');
        $objActSheet->setCellValue("J" . $rowNum, '调拨结果');
        ...
        foreach ($data as $key => $val) {
            ...
            $objActSheet->setCellValue('B' . $rowNum, $val["s_t_s_sn"]);
            ...
            $objActSheet->setCellValue("J" . $rowNum, $val["s_t_s_status_name"]);
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A2:K' . (string)($rowNum - 1))->applyFromArray($styleArray);//应用边框
     ...
     * @param $fname 文件名
     */
    public function createSingleStoreAssignmentApplicationDetailExcel($data, $title, $fname)
    {
        if (!$fname) {
            return false;
        }
        ...
        //数据标题
        $rowNum = 2;
        $objActSheet->setCellValue('A' . $rowNum, "调拨单号");
        $objActSheet->setCellValue('B' . $rowNum, $maindata["s_t_s_sn"]);
        ...
        $objActSheet->setCellValue('D' . $rowNum, '出货仓库');
        $objActSheet->setCellValue('E' . $rowNum, $maindata["store_name2"]);
        $objActSheet->setCellValue('G' . $rowNum, '申请仓库');
        $objActSheet->setCellValue('H' . $rowNum, $maindata["store_name1"]);

        $rowNum = 5;
        $objActSheet->setCellValue('A' . $rowNum, '来源');
        $objActSheet->setCellValue('B' . $rowNum, $maindata["s_t_s_type_name"]);

        $rowNum = 6;
        $objActSheet->setCellValue('A' . $rowNum, '单据状态');
        $objActSheet->setCellValue('B' . $rowNum, $maindata["s_t_s_status_name"]);
        ...
        $objActSheet->setCellValue('I' . $rowNum, '发货数量');
        ...
        foreach ($list as $key => $val) {
            //`is_pass` int(1) DEFAULT '0' COMMENT '当前商品申请是否通过：0.新增，1.未通过，2.已通过',
            ...
            $objActSheet->setCellValue('H' . $rowNum, $val["pass_num"]);
            $objActSheet->setCellValue('I' . $rowNum, $val["remark"]);
            $objActSheet->setCellValue('J' . $rowNum, $val["is_pass_name"]);
            $rowNum++;
        }
        $objActSheet->getColumnDimension('A')->setWidth(12);//设置A列宽度
        $objActSheet->getColumnDimension('B')->setWidth(24);
